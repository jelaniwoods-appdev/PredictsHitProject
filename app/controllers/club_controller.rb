class ClubController < ApplicationController
  ...
  def show_clubs
    @membership_rows = Membership.where({ :user_id => current_user.id, :goes_to => "clubs_table" }).order({ :id => :desc })
  ...
  def view_club
    ...
    @membership_rows = Membership.where({ :club_id => club_id, :goes_to => "clubs_table"}).order_as_specified(category: ["owner", "admin", "member"])

    #relevant comments
    @club_messages_all = Chat.where({ :club_id => club_id, :goes_to => "club" }).order({ :created_at => :desc })
